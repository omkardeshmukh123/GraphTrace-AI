"""
M2 — Knowledge Graph Engine — Graph Writer Plugin
==================================================
Owner: Member 2 (Knowledge Graph)

This module is the M2 side of the M3 graph-writer plugin interface.
M3's Neo4jGraphStore calls this via the plugin system:
    GRAPHTRACE_GRAPH_WRITER=backend.knowledge_graph.graph_writer:write_graph

What it does:
  - Receives the shared ArtifactGraph (nodes + relationships) produced by M3.
  - Writes the entire graph to Neo4j in a single transaction.
  - Uses M2's established Neo4j connection pool (knowledge_graph/connection.py).
  - Enforces the entity_identity uniqueness constraint on first write.
  - Rejects duplicate projects (HTTP 409) and rolls back on any failure.

Neo4j schema applied by this module:
    CREATE CONSTRAINT entity_identity IF NOT EXISTS
    FOR (n:Entity) REQUIRE (n.project_id, n.id) IS UNIQUE;

All writes go through M2's Neo4j driver singleton — no separate connection
management is needed in M3's app layer.
"""
...
def _write_transaction(tx, graph: ArtifactGraph) -> None:
    ...
    project_node = next(
        (node for node in graph.nodes if node.id == graph.project_id), None
    )
    ...
    other_nodes = [node for node in graph.nodes if node.id != project_id]
    batch_size = 200
