...
M1 → M3 Parser Plugin
======================
Implements the parser interface defined in docs/INTEGRATION.md.

M3's analysis pipeline calls this via the plugin system:
    GRAPHTRACE_PARSER=backend.app.parsers.pipeline:parse_project

This function:
  1. Runs our M1 ArtifactAnalyzer on the repository_root
  2. Converts M1's Entity/Relationship output → M3's ArtifactGraph (Node/Relationship)
  3. Parses requirements_text (Markdown SRS) into REQUIREMENT nodes if provided

M3's contract (backend/app/models.py):
  - Exactly one PROJECT node with id == project_id
  - NodeType: PROJECT, FILE, PACKAGE, MODULE, CLASS, INTERFACE, METHOD, FUNCTION,
               REQUIREMENT, DOCUMENT, TECHNOLOGY
  - RelationshipType: CONTAINS, IMPORTS, CALLS, EXTENDS, IMPLEMENTS, DEPENDS_ON,
                      IMPLEMENTED_BY, DOCUMENTED_BY, USES, PART_OF
  - Node IDs: generated via make_node_id(project_id, node_type, reference)
  - IDs must be 1–160 chars, pattern ^[A-Za-z0-9][A-Za-z0-9_.:-]{0,159}$
"""
...
# M1 imports — use full package path since project root is in pythonpath
# and backend/ has __init__.py making it a package
...
def parse_project(
    *,
    ...
    """
    Entry point called by M3's analysis pipeline.
    Returns an ArtifactGraph conforming to backend/app/models.py.
    """
    ...
    if not any(n.id == project_id for n in m3_nodes):
        ...
        # Ensure source property is set on the PROJECT node
        for node in m3_nodes:
            if node.id == project_id:
                node.properties["source"] = "repository"
                break
